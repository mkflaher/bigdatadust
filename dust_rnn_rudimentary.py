...
def forwardprop(X,cell,weights):
    ...
    #return yhat
    forget = tf.nn.tanh(tf.matmul(X,weights['forget']))
    inp = tf.nn.tanh(tf.matmul(X,weights['input']))
    cand = tf.nn.tanh(tf.matmul(X,weights['candidate']))
    out = tf.nn.tanh(tf.matmul(X,weights['out']))
...
def main(): #this will do all the work
    ...
    #w3 = init((nNodes,2))
    weights = {'forget':init((pclen+2,2)),'input':init((pclen+2,2)),'candidate':init((pclen+2,2)),'out':init((pclen+2,2))}
    print(pclen)
    ...
    yhat,cell_state = forwardprop(X,cell,weights)
